...
socket.on('gameStart', ({ youAreFirst, deck, userHand, opponentHand, topCard }) => {
  ...
  initGameFromServer(deck, userHand, opponentHand, topCard);
});
...
socket.on('opponentMove', (move) => {
  if (move.type === 'drop') {
    ...
    window.pickCardsLocal(move.num, false); // opponent pick → show back only
  }
});
...
(() => {
  ...
  let animationCards = [], animationTimer = 0, dealCounter = 0;
  let allCards = [], userCards = [], opponentCards = [], topCard = null;
  let recentPlayedCards = [];
  let shouldEat = false, gameDone = false, isAnimating = false;
  let dragCard = null;
  ...
  window.showMessage = function(txt, color = "#0f0", time = 0) {
    ...
    if (time > 0) {
      ...
      window.msgTimeout = setTimeout(() => messageEl.textContent = "", time);
    }
  };
  ...
  function drawFan(cards,baseY,isUser=true){
    ...
    for(let i=0;i<cards.length;i++){
      ...
      const isHidden = !isUser && (card.suit === null || card.num === null);
      if(inKadi){
      ...
      drawCard(card,x,baseY+dy,scale,card.ready||inKadi,angle,isHidden);
    }
  }
  ...
  function draw(){
    ...
    if (allCards.length > 0 && gameState !== "loading") {
    ...
    if (isAnimating && window.animCards) {
      ...
      window.animCards.forEach((c, i) => {
        ...
        const isHidden = !window.animIsUser && (c.suit === null || c.num === null);
        drawCard(c, x, y, 1, true, ease * 0.3, isHidden);
      });
      window.animStep++;
  ...
  // SERVER INIT – opponentHand is now [null,null,null,null]
  window.initGameFromServer = function(deck, userHand, opponentHand, topCardData) {
    playSound("shuffle");
    
    allCards = deck.map(c => ({ suit: c.suit, num: c.num }));
    userCards = userHand.map(c => ({ ...c, float: false, ready: false }));
    
    // opponentHand is now array of nulls → we show only backs
    opponentCards = opponentHand.map(() => ({ suit: null, num: null }));
    ...
    gameState = "ready";
    myTurn = isMyTurn;
    isAnimating = false; gameDone = false; shouldEat = false;
    winScreen.classList.remove("show");
    
    window.showMessage(myTurn ? "ZAMUU YAKO!" : "Subiri mpinzani...", myTurn ? "#0f0" : "#ff0");
  };

  // DROP LOCAL
  window.dropCardsLocal = function(cards, isUser) {
    if(!isValid(topCard,cards,shouldEat)){
      if(isUser){ playSound("badmove");vibrate();window.showMessage("HAPANA! Si halali!","#f00",3000);userCards.forEach(c=>c.ready=false); }
      return false;
    }
    ...
    let firstIdx = 0;
    if(isUser){
      firstIdx = arr.findIndex(c=>c.suit===cards[0].suit&&c.num===cards[0].num);
    }
    ...
    playSound("throw");
    window.showMessage(isUser?"Poa sana!":"Mpinzani ametupa!","#0f0");
    ...
    setTimeout(()=>{
      ...
      if(isUser){
        ...
        userCards.forEach(c=>{c.ready=false;c.float=false;});
      } else {
        // Remove from opponent (we don't know which, so just remove count)
        opponentCards.splice(0, cards.length);
      }

      const feeder=[1,2].includes(topCard.num);
      if(isQuestion(topCard)){ window.showMessage("FUNIKA HII KADI!","#ff0"); }
      else if(feeder){ 
        window.showMessage(topCard.num===1?"KULA 2!":"KULA 3!","#f00"); 
        shouldEat=true; 
        myTurn=!isUser; 
      }
      else{ 
        shouldEat=false; 
        myTurn=!isUser; 
        window.showMessage(myTurn?"Zamuu yako!":"Mpinzani anacheza...",myTurn?"#0f0":"#ff0",2000); 
      }
      checkGameOver(); 
      isAnimating=false;
    },900);
  ...
  // PICK LOCAL – OPPONENT PICK SHOWS BACK ONLY
  window.pickCardsLocal = function(num, isUser){
    if(isAnimating||gameState!=="ready") return;
    isAnimating=true; 
    window.animCards = [];

    if (!isUser) {
      // Opponent picking → show card backs only
      for(let i = 0; i < num; i++) {
        window.animCards.push({ suit: null, num: null });
      }
    } else {
      // Your pick → real cards
    ...
    window.animStep = 0;
    playSound("pick");
    window.showMessage(isUser ? "Umechukua!" : "Mpinzani anachukua", "#fff");

    setTimeout(() => {
      if (isUser) {
        userCards.push(...window.animCards.map(c => ({...c, float:false, ready:false})));
      } else {
        // Add hidden cards to opponent
        for(let i = 0; i < num; i++) {
      ...
      shouldEat = false;
      myTurn = !isUser;
      window.showMessage(myTurn ? "Zamuu yako!" : "Subiri kidogo...", myTurn ? "#0f0" : "#ff0", 2000);
      checkGameOver();
    }, 900);
  };

  function isQuestion(card){return card && [7,10,11,12].includes(card.num);}
  function canFinish(card){return card && ![0,1,2,7,10,11,12].includes(card.num);}
  ...
  function checkGameOver(){
    if(userCards.length===0 && topCard && canFinish(topCard) && opponentCards.length>0){
    ...
    if(opponentCards.length===0 && topCard && canFinish(topCard) && userCards.length>0){
  ...
  function getHit(x,y){
    ...
    for(let i=userCards.length-1;i>=0;i--){
      ...
      const left=cx+CARD_W*0.05, right=cx+CARD_W*0.95;
      const top=USER_Y+dy, bottom=USER_Y+dy+CARD_H;
      if(x>=left&&x<=right&&y>=top&&y<=bottom) return {type:"user",idx:i};
    }
  ...
  function handleEnd(x,y){
    const hit=getHit(x,y);
    if(hit?.type==="restart"){ socket.emit('rematch', { roomId }); return; }
    if(!myTurn||isAnimating||gameDone||gameState!=="ready") return;
    if(hit?.type==="deck"){
  ...
  canvas.addEventListener("mousedown", e => { const p=getInputXY(e); handleStart(p.x,p.y); });
  ...
  function pickCards(num, isUser) {
    ...
    window.pickCardsLocal(num, isUser);
  }
